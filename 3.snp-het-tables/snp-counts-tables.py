...
def process_vcf(vcf_file, min_depth, min_af, homo_af, exclude_samples):
    sample_data = {}
    ...
    for sample_name in vcf.header.samples:
        if sample_name not in exclude_samples:
            sample_data[sample_name] = {
                ...
                "homozygous_snps": 0,
                "heterozygous_snps": 0,
                "positions": []  # To store SNP positions, AF, and DP for each sample
            }
        ...
        for sample_name in record.samples.keys():
            ...
            if sample.get("DP") is not None and sample["DP"] >= min_depth:
                ...
                if af is not None and isinstance(af, float) and gt_tuple in [(1, 0), (0, 1), (1, 1)]:
                    ...
                    # Save SNP position, AF (formatted), and DP for each SNP
                    sample_data[sample_name]["positions"].append((record.pos, format(af, ".4f"), sample["DP"]))

    return sample_data


def write_output(output_file, results, min_af, homo_af, min_depth):
    with open(output_file, "w") as out:
        ...
        out.write(f"SNP Depth used: {min_depth}\n")
...
def write_sample_snp_details(sample_data, output_folder):
    ...
    for sample, data in sample_data.items():
        ...
        with open(snp_details_file, "w") as snp_file:
            snp_file.write("Position\tAF\tDepth\n")
            for pos, af, dp in data["positions"]:
                snp_file.write(f"{pos}\t{af}\t{dp}\n")
...
def main():
    ...
    write_output(args.output, results, args.af, args.homo_af, args.dp)
